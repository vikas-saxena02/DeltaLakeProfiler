...
// MAGIC %sql
// MAGIC create widget text databaseNames default 'test_vikas_framework'
...
var dbNames = if(databaseNames.trim.size ==0 ) spark.sql("show databases").collect.map(row => row.getString(0)).toList else databaseNames.split(",").toList
...
// MAGIC %sql
// MAGIC select * from physicalTableStats
